...
def test_requeue_orphans_puts_dead_workers_questions_back_in_fifo_order(live_store):
    # A redeploy killed the worker mid-question: the row stayed `thinking` forever and the stage
    # showed "the fly is deciding... 22 min" to everyone (2026-09-22).
    done = live_store.enqueue("Q0?", ip_hash="h1")
    live_store.claim_next()
    live_store.record_answer(done, {"id": done, "answer": "no"})
    orphan = live_store.enqueue("Q1?", ip_hash="h1")
    waiting = live_store.enqueue("Q2?", ip_hash="h1")
    live_store.claim_next()

    assert live_store.requeue_orphans() == [orphan]

    row = live_store.get(orphan)
    assert row["status"] == "queued"
    assert row["started_at"] is None
    assert live_store.get(done)["status"] == "answered"
    assert live_store.claim_next()["id"] == orphan  # back at the front, ahead of Q2
    assert live_store.get(waiting)["status"] == "queued"


def test_requeue_orphans_is_a_noop_on_a_clean_start(live_store):
    live_store.enqueue("Q1?", ip_hash="h1")
    assert live_store.requeue_orphans() == []
