...
def test_ask_accepts_and_stores_a_question_with_an_invisible_leading_character(client):
    # 2026-09-22: a visitor sent this three times, got "not a yes/no question" each time, then
    # hit the rate limit. The stored text is the one without the invisible character.
    resp = client.post(
        "/api/ask", json={"question": "\u200bWill humans walk on Mars before 2030?", "turnstile_token": "x"}
    )
    assert resp.status_code == 202
    import bioreservoir.live.config as live_config
    ...
    with LiveStore(path=live_config.LIVE_DB) as s:
        assert s.get(resp.json()["id"])["question"] == "Will humans walk on Mars before 2030?"
