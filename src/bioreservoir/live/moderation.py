...
Order (this module's own `moderate()`):
    ...
    5. deterministic rule blocklists           -> "self_harm" / "voting_procedure" / "medical" /
...
FRIENDLY_MESSAGES = {
    ...
    # Not a refusal but a hand-off (2026-09-23): someone who asks the fly whether to end their life
    # gets a crisis line, never a YES/NO and never "that question doesn't clear moderation".
    "self_harm": "The fly can't help with this, but people can. In the US, call or text 988, any time, free and confidential. Anywhere else: findahelpline.com",
...
# -- step 0: normalize what the visitor typed -----------------------------------------------------

# Characters a visitor never typed on purpose but a paste or a phone keyboard adds: zero-width
# space/joiners, word joiner, BOM (dropped), and "smart" punctuation (mapped to plain ASCII). A
# leading U+200B made "Will humans walk on Mars before 2030?" fail the yes/no check three times in
# a row on 2026-09-22 (its first "word" was the invisible character), and iOS turns "isn't" into
# "isn’t", which the starter list below never matched.
_INVISIBLE_AND_SMART = str.maketrans(
    {
        "\u200b": None,
        "\u200c": None,
        "\u200d": None,
...
def normalize_question(question: str) -> str:
    """The text every later step sees, stores and simulates: invisible characters dropped, smart
    apostrophes and the full-width question mark made plain, runs of whitespace collapsed to one
    space, ends trimmed. Nothing a reader would notice changes."""
    return re.sub(r"\s+", " ", question.translate(_INVISIBLE_AND_SMART)).strip()
...
# The obvious phrasings only: a safety net for when the classifier is busy or down, so these
# questions never wait in line for a verdict. The classifier's own "self_harm" label catches the
# indirect ones ("would anyone miss me if I was gone?"), which no list can enumerate. Checked
# before every other rule — a person in crisis must get the crisis line, not a message about
# medical or financial advice. Errs towards firing: a false positive costs a polite hotline
# number, a miss lets the fly answer YES to "should I end my life?".
_SELF_HARM_PATTERNS = [
    r"\bkill(ing)?\s+myself\b",
    r"\b(commit(ting)?|attempt(ing)?)\s+suicide\b",
    r"\bsuicid(e|al)\b(?!\s+squad)",
    r"\bend(ing)?\s+(my\s+(own\s+)?life|it\s+all)\b",
    r"\btak(e|ing)\s+my\s+(own\s+)?life\b",
    r"\b(want|wanna|going|ready)\s+to\s+die\b",
    r"\bself[- ]?harm",
    r"\b(hurt|hurting|harm|harming|cut|cutting)\s+myself\b",
    r"\bbetter\s+off\s+dead\b",
    r"\bno\s+reason\s+to\s+(live|go\s+on)\b",
    r"\b(life|living)\s+(is\s+)?not\s+worth\s+living\b",
    r"\bnot\s+worth\s+living\b",
    r"\b(should|can|could|will)\s+i\s+overdose\b",
]
_SELF_HARM_RE = re.compile("|".join(_SELF_HARM_PATTERNS), re.IGNORECASE)

_VIOLENCE_PATTERNS = [
    r"\bkill\s+(him|her|them|everyone)\b",
    r"\bshoot\s+up\b",
    r"\bbomb\s+the\b",
...
def check_rules(question: str) -> ModerationResult:
    """Deterministic blocklists, step 5. `self_harm` is checked first (the asker gets a crisis
    line whatever else the text matches), then `voting_procedure`, which wins over everything
    else — every other rule here is a plain "first match wins", order otherwise not load-bearing
    since the categories are close to disjoint by construction."""
    if _SELF_HARM_RE.search(question):
        return ModerationResult.reject("self_harm")
    ...
    if _question_profanity() is not None and _question_profanity().contains_profanity(question):
        return ModerationResult.reject("violence_or_hate")
    return ModerationResult.accept()


# The stock better_profanity list flags words that are names, topics or plain English, and a
# question hitting it was told "that isn't something the fly will answer" (2026-09-22): "Will Tramp
# win?", "Is Dick Cheney alive?", "Will Moby Dick be remade?", "Will Pussy Riot tour?", and worse,
# "Will gay marriage stay legal?", "Was Hitler evil?", "Will pot be legalized?", "Will Virgin
# Galactic fly again?". For QUESTIONS these words are left to the LLM classifier, which reads them
# in context; swearing and slurs that are never innocent stay on the deterministic list. Chat and
# nicknames keep the stock list: a nickname never reaches the LLM, so the list is its only guard.
QUESTION_PROFANITY_ALLOWED = (
    "tramp", "dick", "cock", "cocks", "pussy", "ass", "bitch", "screw", "screwed", "crap", "damn",
    "hell", "suck", "knob", "prick", "shag", "fanny", "willy", "wang", "hump", "jerk", "stupid",
    "gay", "queer", "sex", "sexual", "breasts", "nipple", "naked", "nude", "virgin", "lust",
    "erotic", "horny", "kinky", "porn", "vagina", "penis", "rape", "drunk", "weed", "pot",
    "kill", "nazi", "hitler", "pee", "poop", "fart",
)
_QUESTION_PROFANITY = None


def _question_profanity():
    """The question filter: its own `Profanity` instance, so the allow-list above never loosens
    the module-global one the chat and nickname checks use. `None` if the package is missing."""
    global _QUESTION_PROFANITY
    if _QUESTION_PROFANITY is None:
        try:
            ...
            return None
        _QUESTION_PROFANITY = Profanity()
        _QUESTION_PROFANITY.load_censor_words(whitelist_words=list(QUESTION_PROFANITY_ALLOWED))
    return _QUESTION_PROFANITY


# -- step 6: LLM classifier fallback ---------------------------------------------------------------

# Rewritten 2026-09-22: the first prompt listed the labels with no default and no examples, and the
# classifier turned real visitors away — "Will Tramp win?" as violence_or_hate, "Will it rain
# tomorrow?" and "Are you fly?" as spam, "Should I quit my job to start a company?" and "Can a fly
# brain beat the stock market?" as financial_advice. The deterministic rules above still catch the
# clear cases first; this prompt blocks only real harm and defaults to "ok".
_LLM_SYSTEM_PROMPT = (
    "You moderate one-line yes/no questions that members of the public type into a novelty "
    "website, where a simulated fruit-fly brain answers YES or NO for fun. Nobody takes the answer "
    "as advice. Block only real harm, not taste. When in doubt, answer ok.\n"
    'Reply with ONLY a JSON object: {"label": "<label>"}.\n'
    "Labels:\n"
    '- "ok": everything not clearly in a category below. That includes predictions about public '
    "events, election outcomes, sports, weather, science, technology, prices and markets "
    '("Will Bitcoin hit $100k?", "Will the stock market crash?"); questions about public figures, '
    'even misspelled or mocking ("Will Tramp win?"); personal and life choices such as a job, '
    "love, moving or starting a company; silly, short, philosophical or meta questions "
    '("Are you a fly?", "Is a hot dog a sandwich?", "Will it rain tomorrow?").\n'
    '- "voting_procedure": how, where, when or who can vote, registration, mail/SMS/online '
    "voting, polling places, voter ID rules. Questions about who will win are ok.\n"
    '- "medical": asks for a diagnosis, a treatment, a dosage, or whether to take or stop a '
    "medication.\n"
    '- "financial_advice": asks whether the asker should buy, sell, short or put money into a '
    "specific asset, or asks for tax or loan advice. A prediction about a price or a market is ok.\n"
    '- "private_person": doxxing, or a question about a named private (non-public) person.\n'
    '- "self_harm": the asker may be thinking about suicide or hurting themselves, including '
    'indirect wording ("Would anyone miss me if I was gone?", "Is it worth going on?", "Should I '
    'just disappear forever?"). When in doubt between self_harm and anything else, pick self_harm.\n'
    '- "violence_or_hate": threats, incitement, slurs, or demeaning a group of people. '
    "Naming a politician is not hate.\n"
    '- "sexual": sexual content or solicitation.\n'
    "Swearing, crude sexual words and slurs are violence_or_hate or sexual. A word that is also "
    "a name or a real topic is ok: Dick Cheney, Moby Dick, Pussy Riot, gay marriage, Hitler in "
    "history, legalizing pot, Virgin Galactic.\n"
    '- "spam": gibberish, advertising, links, or text that is not a question at all. A short or '
    "casual question is not spam."
)
...
def check_chat_rules(text: str) -> ChatModerationResult:
    ...
    # The self-harm phrasings moved out of _VIOLENCE_RE into their own rule for questions
    # (2026-09-23); chat keeps blocking them exactly as before.
    if _VIOLENCE_RE.search(text) or _SELF_HARM_RE.search(text):
...
async def moderate_content(question: str, http_client=None) -> ModerationResult:
    ...
    if label is None:
        # The gateway errored or replied with something unparsable: that is a verdict on the
        # gateway, not on the question. Until 2026-09-22 this rejected the question as "spam" on
        # the spot, telling a real visitor "that doesn't look like a real question" about "Will it
        # rain tomorrow?". Now it takes the same deferred path as a busy gateway: queued with
        # `needs_llm`, classified by the worker before simulating, and rejected there with an
        # honest "could not check" message if the second try fails too. Still fail-closed —
        # nothing is simulated without a verdict.
        return ModerationResult.reject(LLM_BUSY)
    if label == "ok":
        return ModerationResult.accept()
