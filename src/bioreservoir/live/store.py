...
class LiveStore:
    ...
    def requeue_orphans(self) -> list[int]:
        """Put every `thinking` row back in line, oldest first as before, and return their ids.
        Called once by the worker at startup: there is only ever one worker, so a row still
        `thinking` when it starts was claimed by a process that died mid-question (a redeploy
        recreates the container). Without this the row stayed `thinking` forever and the live
        stage told every visitor "the fly is deciding... 22 min" (2026-09-22). FIFO order is
        kept because `claim_next` picks the lowest id and these rows keep theirs."""
        self._conn.execute("BEGIN IMMEDIATE")
        try:
            ids = [
                r["id"]
                for r in self._conn.execute(
                    "SELECT id FROM live_questions WHERE status = 'thinking' ORDER BY id"
                ).fetchall()
            ]
            self._conn.execute(
                "UPDATE live_questions SET status = 'queued', started_at = NULL WHERE status = 'thinking'"
            )
            self._conn.commit()
        except Exception:
            self._conn.rollback()
