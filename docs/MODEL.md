...
**A denser public BANC does exist, and this model does not use it yet** (found 2026-09-21,
correcting an earlier claim here that it did not): the deposit we load from,
Dataverse doi:10.7910/DVN/8TFGGB v8.1 (CC BY 4.0), does hold only materialization 626, but a
second deposit — doi:10.7910/DVN/7WTH1N, "Publication version: Distributed control circuits
across a brain-and-cord connectome", v3.0, CC BY 4.0, 379 files (verified against the Dataverse
API) — ships **materialization 888**, including `banc_888_edgelist_simple_v3.feather` and
`banc_888_meta.feather`. Whether the extra proofreading in 888 fixes the female readout's failed
side-tracking validation is an open question, not an assumption: it has to be re-run and
re-reported either way. Live BANC via CAVE still needs a restricted key — that part is unchanged.
...
flagged, and excluded from every headline claim. Materialization 888 (above) is the next thing
to try; until it is run and reported, the exclusion stands.
