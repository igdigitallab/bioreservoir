...
rule blocklists (`self_harm` first: the obvious phrasings get a crisis line — 988 in the US,
findahelpline.com elsewhere — instead of a refusal, and the LLM has the same label for indirect
wording; then `voting_procedure`, a **hard block** that wins over everything else including the LLM;
